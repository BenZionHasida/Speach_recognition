...
export function App() {
  ...
  let recordNextMishna = ()=>{
    if (currentMishnaData[1]+1 <= currentMishnaData[3]){
        ...
        async function fetchNextMishna(){
          let response = await axios.get("https://www.sefaria.org/api/texts/"+currentMishnaData[0])
  ...
  let recordNextPerek = ()=>{
    let perekAsNumber = Number(currentMishnaData[0][currentMishnaData[0].length - 1]) 
    let pathOfFetch = currentMishnaData[0].slice(0,-1)
    if(perekAsNumber + 1 <= currentMishnaData[2]){
        async function fetchNextPerek(){
          let response = await axios.get("https://www.sefaria.org/api/texts/"+pathOfFetch+(perekAsNumber+1))
          let data = response.data
          setOriginalString(data.he[0])
          setRouter('Record')
          let newData = [pathOfFetch+(perekAsNumber+1),1,currentMishnaData[2],data.he.length]
          setCurrentMishnaData(newData)
