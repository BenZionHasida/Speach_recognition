...
function MasechetSelector(props) {
    ...
    async function fetchMasecet() {
      ...
      for (let index = 0; index < 6; index++) {
        if (data[1].contents[index].heCategory === props.seder) {
          ...
          for (
            let index1 = 0;
            index1 < data[1].contents[index].contents.length - 1;
            index1++
          ) {
            let heTitle = data[1].contents[index].contents[
              index1
            ].heTitle.replace("משנה", "מסכת");
            boxOfNames.push(heTitle);
            boxDictionaryOfNames[heTitle] =
              data[1].contents[index].contents[index1].title;
          }
  ...
  function handleChange(event) {
    props.sendMasecet(
      event.target.value,
      dictionaryOfNames[event.target.value]
    );
  }
